...
;(function () {
  ...
  const SHARED_UI_CSS = `
    ...
    .vw-header-icon {
        ...
        border: 2px solid #1e2be8 !important;
    }
    .vw-main-content {
        display: flex !important;
        flex-direction: column !important;
    ...
    .vw-substatus {
        ...
        color: rgba(207,214,230,0.82) !important;
        text-align: center !important;
    ...
    .vw-btn:hover { opacity: 0.92 !important; transform: translateY(-2px) !important; }
    .vw-btn:disabled { opacity: 0.45 !important; cursor: not-allowed !important; transform: none !important; }
  ...
  function injectUI() {
    ...
    const paint = checked => {
      if (track) track.style.background = checked ? 'linear-gradient(135deg, #0f1b4f, #1e2be8)' : 'rgba(255,255,255,0.20)'
    ...
    if (redirectParam) {
      ...
      if (rp.includes('https://flux.li/android/external/main.php')) {
        document.documentElement.innerHTML =
          ...
          rp +
          '" style="color:#cfd6e6;background:linear-gradient(90deg,#0f1b4f,#1e2be8);padding:10px 14px;border-radius:10px;text-decoration:none;font-weight:700;border:1px solid rgba(255,255,255,0.14);display:inline-block">Click here to continue</a></div></div></body></html>'
        return
      }
